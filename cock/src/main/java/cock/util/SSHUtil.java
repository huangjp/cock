...
public class SSHUtil {
			...
			for (String key : map.keySet()) {
				if (fieldname.toLowerCase().equals(key.toLowerCase())) {
					Method m = t.getClass().getDeclaredMethod(
							"set" + initcap(fieldname),
							new Class[] { (Class<?>) type });
					if (m == null)
						break;
					if ("class java.lang.String".equals(type.toString())) {
						m.invoke(t, new Object[] { map.get(key).toString() });
						break;
					}
					m.invoke(t, new Object[] { map.get(key) });
	...
	public static <T> String getClassAttrs(T entity)
			throws IllegalArgumentException, IllegalAccessException,
			InvocationTargetException, SecurityException, NoSuchMethodException {
	...
	public static void main(String[] args) {
		...
		System.out.println(getString(date));
	}
	...
	public static Class<?> getClass(String path, String key) {
		...
		try {
			prop.load(SSHUtil.class.getClassLoader().getResourceAsStream(path));
